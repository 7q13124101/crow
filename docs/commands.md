...
```bash
cd backend
uv run crow <group> <command> [options]
```

### Server Commands

```bash
uv run crow server run              # Start dev server
uv run crow server run --reload     # With hot reload
uv run crow server run --port 9000  # Custom port
uv run crow server routes           # Show all registered routes
```

### Database Commands

```bash
uv run crow db init                  # Run all migrations
uv run crow db migrate -m "message"  # Create new migration
uv run crow db upgrade               # Apply pending migrations
uv run crow db upgrade --revision e3f  # Upgrade to specific revision
uv run crow db downgrade             # Rollback last migration
uv run crow db downgrade --revision base  # Rollback to start
uv run crow db current               # Show current revision
uv run crow db history               # Show migration history
```
...
uv run crow user create

# Create user non-interactively
uv run crow user create --email user@example.com --password secret

# Create user with specific role
uv run crow user create --email admin@example.com --password secret --role admin

# Create user with superuser flag
uv run crow user create --email admin@example.com --password secret --superuser

# Create admin (shortcut)
uv run crow user create-admin --email admin@example.com --password secret

# Change user role
uv run crow user set-role user@example.com --role admin

# List all users
uv run crow user list
```

### Celery Commands

```bash
uv run crow celery worker                    # Start worker
uv run crow celery worker --loglevel debug   # Debug logging
uv run crow celery worker --concurrency 8    # 8 worker processes
uv run crow celery beat                      # Start scheduler
uv run crow celery flower                    # Start Flower UI
uv run crow celery flower --port 5556        # Custom Flower port
```
...
```bash
uv run crow cmd <command-name> [options]
```
...
```bash
uv run crow cmd my-command --name test
```
