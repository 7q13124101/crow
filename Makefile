...
quickstart: install docker-up
	...
	cd backend && uv run crow db migrate -m "initial" || true
	@echo "Applying migrations..."
	cd backend && uv run crow db upgrade
	@echo "Creating admin user (admin@example.com / admin123)..."
	cd backend && uv run crow user create --email admin@example.com --password admin123 --superuser 2>/dev/null || true
	@echo ""
...
db-migrate:
	...
	uv run --directory backend crow db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend crow db upgrade

db-downgrade:
	uv run --directory backend crow db downgrade

db-current:
	uv run --directory backend crow db current

db-history:
	uv run --directory backend crow db history

# === Server ===
run:
	uv run --directory backend crow server run --reload

run-prod:
	uv run --directory backend crow server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend crow server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend crow user create-admin

user-create:
	uv run --directory backend crow user create

user-list:
	uv run --directory backend crow user list

# === Celery ===
celery-worker:
	uv run --directory backend crow celery worker

celery-beat:
	uv run --directory backend crow celery beat

celery-flower:
	uv run --directory backend crow celery flower
	@echo ""
...
docker-db:
	...
	@echo "   Connection: postgresql://postgres:postgres@localhost:5432/crow"
